...
class AnalyseDream:
    ...
    def set_subject_of_dream(self):
        ...
        client_subjects = ["i", "me"]
        someone_subjects = ["he", "she", "him", "her", "friend", "parent", "mum", "mother", "dad", "father"]
        other_subjects = ["it", "other", "car"]

        #TODO: Setting subjects needs to be refined!!!
        # for sent in self.dream.get_latest_message().sents:
        #     for token in sent:
        #         if DEBUG_ENABLED:
        #             print("{} has a lemma of {}".format(token.text, token.lemma_))
        #         if  token.dep_ == "nsubj":
        #             if token.pos_ == "NOUN":
        #                 self.dream.set_subject(Subject.SOMEONE)
        #                 return Subject.SOMEONE
        #             if token.pos_ == "PRON":
        #                 if token.lemma_ in someone_subjects:
        #                     self.dream.set_subject(Subject.SOMEONE)
        #                     return Subject.SOMEONE
        #                 if token.text in client_subjects:
        ...
        for sent in self.dream.get_latest_message().sents:
            for token in sent:
                ...
                if (token.text.strip(".").lower() in client_subjects):# and (self.dream.get_subject() is None):
                    self.dream.set_subject(Subject.CLIENT)

                    #return self.dream.get_subject()
                if (token.text.strip(".").lower() in someone_subjects) and (self.dream.get_subject() is None):
                    self.dream.set_subject(Subject.SOMEONE)
                    if DEBUG_ENABLED:
                        print("Subject = {}".format(self.dream.get_subject()))
    ...
    def topic_rating_dirobj_description_of(self, tokens: List[Token]):
        ...
        for token in opinion_essence:
            if (token._.topic is not None):
                if not self.dream.get_direct_object():
    ...
    def opinions(self, feedback: Doc):
        ...
        for tokens in feedback.sents:
            if DEBUG_ENABLED:
                ...
                print("TODO: parameter tokens: {}".format(tokens))
            self.topic_rating_dirobj_description_of(tokens)
